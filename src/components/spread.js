...
class Spread extends Component {
  state = {
    ...
    arrayCsv: [],
    created_byid: "",
    b_clientsprd: [], //client select
    b_client_idsprd: "",
    s_clientsprd: [], //client Sell select
    s_client_idsprd: "",
    b_tradersprd: [],
    s_tradersprd: [],
    b_accountssprd: [],
  ...
  handleChangeBsprd = x => e => {
    this.setState({ [x]: e.target.value });
    let client = e.target.value;
    for (let i = 0; i < this.state.value.length; i++) {
      if (this.state.value[i].clients === client) {
        this.setState({
          b_client_idsprd: this.state.value[i].id,
          b_tradersprd: this.state.value[i].traders[0],
          b_accountssprd: this.state.value[i].accounts[0],
          b_commssprd: this.state.value[i].commission,
          b_recapsprd: this.state.value[i].recap_emails,
          b_idbsprd: this.state.value[i].idb
        });
      }
    }
  };
            ...
            <div className={classes.midbutton}>
              <FormControl className={classes.formControl} variant="outlined">
                ...
                <Select
                  ...
                  native
                  value={this.state.b_clientsprd}
                  onChange={this.handleChangeBsprd("b_clientsprd")}
                  input={
                    <OutlinedInput
                      name="b_clientsprd"
                      labelWidth={this.state.labelWidth}
